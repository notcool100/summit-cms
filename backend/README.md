...
A .NET 10 Clean Modular Monolith backend for the Summit marketing site, with full admin CRUD over every
...
- owns its **own EF Core `DbContext`**, mapped to its **own PostgreSQL schema** (`identity`, `media`,
  `content`, `company`, `capabilities`, `industries`, `projects`, `contact`) and its own
  migrations folder, so modules can be migrated/evolved independently even though they share one database;
...
2. Seeds (idempotently, so safe to run on every startup): system roles (`SuperAdmin`, `Admin`, `Editor`,
   ...
   enquiry types) ported from the old static frontend data files so the database
   isn't empty on day one.

The seeded admin's password is either read from `Seed:AdminPassword` config, or (if unset) randomly
generated and printed **once** to the startup log, copy it immediately, as it is never shown again. Set
...
any hardcoded value. Set it first: `export SUMMITCMS_CONNECTION="Host=...;Port=...;Database=...;Username=...;Password=..."`.)
